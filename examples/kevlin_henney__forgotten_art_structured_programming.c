...
int year;
{
	...
	offset = ((int)log10(year) +1) -2 ;	
	/* +1 to fix log10 origin, +2 to skip to tens of years*/
...
int main() {
	...
	printf("Function:	year  isleap1  isleap2  isleap3  isleap4  isleap5  isleap6  isleap7  EXPECTED\n");

	for (i = 0; i < no_testcases ; i ++ ){

		printf("Results:    %8d %5d %8d %8d %8d %8d %8d %8d %8d\n", testcases[i],
			isleap1(testcases[i]), isleap2(testcases[i]), isleap3(testcases[i]), 
			isleap4(testcases[i]), isleap5(testcases[i]), 
			isleap6(testcases[i]), isleap7(testcases[i]), actual_result[i] ); 
...
/*
	...
	This re-entrant C code is from a telecoms application.
		...
		"I feel a combination of pride and revulsion at
		this discovery"
							~ Tom Duff

		"Many people have said the worst feature of C is
		that switches don't break automatically before each
		case statement. This code forms some sort of arguement
		in that debate, but I am not sure whether it is for
		or against."
							~ Tom Duff
	...
	But the problem is that this applies only to FORTRAN66 and
	doesn't really apply to almost all other languages (ie
	Algol/X based languages).

	The problem with the return statement is that it violates
	the "single responsibility rule" as it does two things: (1)
	return binds a value to the result of a function,
